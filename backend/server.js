...
app.post('/api/register-id', async (req, res) => {
  ...
  // NEW: Check if identity already exists for this email
  try {
    const { data: existingProfile, error: checkError } = await getProfileByEmail(email);
    if (checkError) {
      console.error('Supabase profile check error:', checkError);
      return res.status(500).json({ error: 'Failed to check existing identity' });
    }
    if (existingProfile) {
      console.log('Identity already exists for email:', email);
      return res.status(409).json({ 
        success: false, 
        message: 'Identity already registered for this email. Use recovery to access it.' 
      });
    }
  } catch (error) {
    console.error('Error checking existing identity:', error);
    return res.status(500).json({ error: 'Database error during identity check' });
  }
